<div class="auditionsPhotos index">
	<h2><?php echo __('Auditions Photos'); ?></h2>
	...
	<?php foreach ($auditionsPhotos as $auditionsPhoto): ?>
	<tr>
		<td><?php echo h($auditionsPhoto['AuditionsPhoto']['id']); ?>&nbsp;</td>
		<td>
		...
		<td><?php echo h($auditionsPhoto['AuditionsPhoto']['created']); ?>&nbsp;</td>
		<td><?php echo h($auditionsPhoto['AuditionsPhoto']['modified']); ?>&nbsp;</td>
		<td>
			<?php echo $this->Html->link($auditionsPhoto['User']['name'], array('controller' => 'users', 'action' => 'view', $auditionsPhoto['User']['id'])); ?>
		</td>
		<td class="actions">
			<?php echo $this->Html->link(__('View'), array('action' => 'view', $auditionsPhoto['AuditionsPhoto']['id'])); ?>
			<?php echo $this->Html->link(__('Edit'), array('action' => 'edit', $auditionsPhoto['AuditionsPhoto']['id'])); ?>
			<?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $auditionsPhoto['AuditionsPhoto']['id']), null, __('Are you sure you want to delete # %s?', $auditionsPhoto['AuditionsPhoto']['id'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
	</table>
