...
<div class="row row-header-h1">
	<div class="col-sm-12">
		<h1><?php echo __('Edit Casting'); ?></h1>
	</div>
</div>

<div class="row">
	<div class="col-sm-12">
		<?php echo $this->Form->create('Casting', array('class' => 'form-horizontal', 'role' => 'form', 'type' => 'file')) ?>
			...
			<div class="form-group">
				...
				<div class="col-sm-8">
					<div class="row row-image-name">
						<div class="col-sm-12">
							<span class="label <?php echo ($this->request->data['Casting']['image']) ? 'label-primary' : 'label-danger'; ?>">
								<?php echo ($imageName = $this->request->data['Casting']['image']) ? $imageName : __('No image'); ?>
							</span>
						</div>
					</div>
					<div class="row">
									...
									<?php foreach ($this->request->data['Photo'] as $key => $photo): ?>
										<span class="label label-primary"><?php echo $photo['name']; ?> | 
											...
											echo $this->Html->Link('x'
												, array('controller' => 'CastingsPhotos'
													, 'action' => 'remove'
													, $photo['CastingsPhoto']['id']
												)
											);
											?></span>
										&nbsp;
			...
			<div class="form-group">
				<label for="element-date" class="col-sm-4 control-label"><?php echo __('Casting date'); ?></label>
				<div class="col-sm-8">
					<div class='input-group date' id='datetimepickerCasting'>
							...
							<?php if(isset($this->request->data['Video']) && sizeof($this->request->data['Video']) > 0): ?>
									<?php foreach ($this->request->data['Video'] as $key => $video): ?>
										<span class="label label-primary"><?php echo $video['name']; ?> | 
											<?php
											echo $this->Html->Link('x'
												, array('controller' => 'CastingsVideos'
													, 'action' => 'remove'
													, $video['CastingsVideo']['id']
												)
											);
											?></span>
										&nbsp;
							...
							<?php if(isset($this->request->data['Attachment']) && sizeof($this->request->data['Attachment']) > 0): ?>
									<?php foreach ($this->request->data['Attachment'] as $key => $attachment): ?>
										<span class="label label-primary"><?php echo $attachment['name']; ?> | 
											<?php
											echo $this->Html->Link('x'
												, array('controller' => 'AttachmentsCastings'
													, 'action' => 'remove'
													, $attachment['AttachmentsCasting']['id']
												)
											);
											?></span>
										&nbsp;
...
<div class="actions">
	...
	<ul>
		...
		<li><?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $this->Form->value('Casting.id')), null, __('Are you sure you want to delete # %s?', $this->Form->value('Casting.id'))); ?></li>
