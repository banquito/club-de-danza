<div class="attachmentsAuditions index">
	<h2><?php echo __('Attachments Auditions'); ?></h2>
	...
	<?php foreach ($attachmentsAuditions as $attachmentsAudition): ?>
	<tr>
		<td><?php echo h($attachmentsAudition['AttachmentsAudition']['id']); ?>&nbsp;</td>
		<td>
		...
		<td><?php echo h($attachmentsAudition['AttachmentsAudition']['created']); ?>&nbsp;</td>
		<td><?php echo h($attachmentsAudition['AttachmentsAudition']['modified']); ?>&nbsp;</td>
		<td>
			<?php echo $this->Html->link($attachmentsAudition['User']['name'], array('controller' => 'users', 'action' => 'view', $attachmentsAudition['User']['id'])); ?>
		</td>
		<td class="actions">
			<?php echo $this->Html->link(__('View'), array('action' => 'view', $attachmentsAudition['AttachmentsAudition']['id'])); ?>
			<?php echo $this->Html->link(__('Edit'), array('action' => 'edit', $attachmentsAudition['AttachmentsAudition']['id'])); ?>
			<?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $attachmentsAudition['AttachmentsAudition']['id']), null, __('Are you sure you want to delete # %s?', $attachmentsAudition['AttachmentsAudition']['id'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
	</table>
